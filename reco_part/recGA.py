...
def classify(year,mark):
    ...
    count = 0 #筛选学校的数目
    # GA中先拿 15-19的做为已知 20的作为验证年份
    N = int(25*100*nortest(mark)) + 25 #调优 21为未知 不使用
    for i in range(len(df)):
        year = 2015
        for j in range(5):#GA时改为5  使用时为6
    ...
    for i in range(len(school)):
        for j in range(len(df)):
            if df['name'][j] == school[i] and df['batch'][j]==school_batch[i]:
                # 放入真实分数线
                real_mark.append(getmark(df[yearP][j],2020))# GA时为2020  使用时改为2021
                # 计算p
                count = 0
                for k in year:
                    if df[k][j] >= rank_s:
