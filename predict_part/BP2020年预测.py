...
def main():
    ...
    #写入数据
    storepath = "BP预测河南2020年的录取排名和误差值.xls"
    #存储数据
    book = xlwt.Workbook(encoding="utf-8", style_compression=0)
    sheet = book.add_sheet("BP预测河南2020年的录取排名和损失值", cell_overwrite_ok=True)
    ...
    while (i < height):
        ...
        try:
            ...
            sum1 = (df.values[i][10] + df.values[i][11] + df.values[i][12] + df.values[i][13] +
                    df.values[i][14]) / 5
            top = 0  # 这个用来记录是否异常
            ...
            train_x0 = np.array([df.values[i][14], df.values[i][13], df.values[i][12]])
            train_x1 = np.array([df.values[i][13], df.values[i][12], df.values[i][11]])
            train_y = np.array([df.values[i][12], df.values[i][11], df.values[i][10]])
            ...
            # 进行结果预测
            futureyear1 = np.array([df.values[i][11]])
            futureyear2 = np.array([df.values[i][10]])
            ...
            loss = abs(networkout1[0][0] -df.values[i][9])
            # print(loss)
